...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function == LossFunction.MSE:
            return (-2/y.shape[0])*x.T @ (y-x@self.w)
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        Y = np.take(y, indexes)
        if self.loss_function == LossFunction.MSE:
            return -2/self.batch_size*X.T @ (Y-X@self.w)
        if self.loss_function == LossFunction.LogCosh:
            return (1/self.batch_size)*X.T @ np.tanh(X@self.w - Y)
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        diff = - self.lr()*m_est/(v_est**0.5 + self.eps)
        self.w = self.w + diff
        return diff
